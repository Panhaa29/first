<?php
    ...
    $cn->set_charset("utf8");
    $last_id = 1;
    $sql = "SELECT id FROM tbl_content ORDER BY id DESC";
    $res = $cn->query($sql);
    $num = $res->num_rows; 
    if($num>0){
        $row = $res->fetch_array();
...
<body>
    ...
    <div class="frm" style="width:800px;">
        <form class="upl">
            ...
            <div style="width:50%; float:left;">
                ...
                <label for="" class="frm-controll" >Menu</label>
                <select name="txt-menu" id="txt-menu" class="frm-controll" >
                    <option value="0">--Select Menu--</option>
                    <?php
                        $sql = "SELECT id,name FROM tbl_menu WHERE status=0";
                        $res = $cn->query($sql);
                        if( $res->num_rows > 0 ){
            ...
            <div class="btn-save" id="btn-save-content">
                Save Changes
            </div>
        </form>
    </div>
    ...
    <table id="tbldata">
        <tr>
            ...
            <th>Description</th>
            <th width="50" >OD</th>
            <th width="150" >status​​​​​​​ (1= Delete)</th>
        ...
        <?php
        $sql = "SELECT tbl_content.*,tbl_menu.name FROM tbl_content INNER JOIN tbl_menu 
        ON tbl_content.menu_id = tbl_menu.id ORDER BY tbl_content.id DESC ";
            ...
            while( $row = $res->fetch_array()){
                ?>
                    <tr>
                        ...
                        <td> <span style="display:none;"> <?php echo $row[4] ?> </span> <?php echo $row[7] ?> </td>
                        ...
                        <td>
                            <a class=" btn-edit" id="btn-edit-ct">
